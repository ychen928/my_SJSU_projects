...
Ensure(motor_value)
{
    ...
    expect(read_sensor_value, when(sensor_value, is_equal_to(3)));
    value = get_motor_pwm(3);
    assert_equal_with_message(value, 8, "Not Equal to 8 %d", value);
    
    expect(read_sensor_value, when(sensor_value, is_equal_to(75)));
    value = get_motor_pwm(75);
    assert_equal_with_message(value, 9, "Not Equal to 9 %d", value);
    
    expect(read_sensor_value, when(sensor_value, is_equal_to(199)));
    value = get_motor_pwm(199);
    assert_equal_with_message(value, -1, "Not Equal to -1 %d", value);
...
TestSuite *motor_pwm_test()
{
    TestSuite *suite = create_test_suite();
    add_test(suite, motor_value);
    return suite;
}
...
int main(int argc, char **argv) 
{
    ...
    add_suite(suite, motor_pwm_test());
